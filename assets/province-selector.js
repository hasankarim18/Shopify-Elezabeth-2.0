    ...
    allAddressesSelector.forEach((select) => {
      ...
      for (var index = 0; index < arrayOfProvince.length; index++) {
        if (
          ...
          options += `<option value="${arrayOfProvince[i][0]}" selected>${arrayOfProvince[index][0]}</option>`;
        } else {
          options += `<option value="${arrayOfProvince[i][0]}">${arrayOfProvince[index][0]}</option>`;
        }
      }
